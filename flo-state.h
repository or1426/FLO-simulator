...
public:
  ...
  static FLOState computational_basis_state(int qubits, int x){
    std::vector<int> x_vec(qubits, 0);
    for(int i = 0; i < qubits; i++){
      x_vec[i] = (x>>i) & 1;
    }
    return computational_basis_state(qubits, x_vec);
  }
